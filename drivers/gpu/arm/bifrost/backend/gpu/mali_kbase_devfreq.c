...
int kbase_devfreq_opp_helper(struct dev_pm_set_opp_data *data)
{
	...
	struct regulator *mem_reg = (data->regulator_count >= 2) ? data->regulators[1] : NULL;
	...
	if (new_freq >= old_freq) {
		if (mem_reg == NULL) {
			dev_err(dev, "No mem reg in opp-table. Number of regulators %d. Failed to set volt %lu uV for mem reg\n",
				data->regulator_count, new_supply_mem->u_volt);
		} else {
